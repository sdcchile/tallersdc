...
El objetivo del siguiente proceso de anonimización es ilustrar el proceso SDC establecido en la [Guía para el Control de la Divulgación Estadística en Microdatos](https://www.ine.gob.cl/docs/default-source/buenas-practicas/directrices-metodologicas/guias-y-orientaciones-metodologicas/documentos/gu%C3%ADa-control-divulgaci%C3%B3n-estad%C3%ADstica-microdatos.pdf?sfvrsn=b6bdd28f_2) propuesta por la Mesa de Anonimización INE. Los datos utilizados corresponden a una base de datos sintética que se basa en el diseño de la 17a ENUSC 2020 [@manualbdenusc].


El proceso SDC aquí implementado busca generar un archivo de datos PUF (del inglés, *Public Use File*), que pueda ponerse a disposición de forma pública y para cualquier usuario que lo requiera. Para ello, el proceso SDC aplicado apunta al logro de los umbrales de riesgo establecido por la "Guía para el Control de la Divulgación Estadística en Microdatos" para encuestas de hogares [@manualsdcine]. Simultáneamente, el ejercicio busca mantener la utilidad de los datos, intentando minimizar la intervención de los mismos para que los usuarios puedan replicar las estadísticas priorizadas por esta encuesta. 
...
yourdirectory<-"~/2023/SDC_traing_julio/simulacion" 
setwd(yourdirectory)
```
...
```
```{r, echo=TRUE}
fname <- "BD_ENUSC_SINTETICA_ETIQUETADA.sav"
file <- haven::read_sav(fname)
```

#### Caracterización de la base de datos

Primero revisamos las dimensiones del archivo, es decir, cuantos registros y variables contiene. La base contiene 47.344 registros y 73 variables.
...
Luego, revisamos el nombre de las variables contenido en la base de datos para verificar que se ajusta a la base con que se requiere trabajar.
...
```
```{r, echo=TRUE, fig.cap='Punto de corte óptimo con datos no tratados', warning=FALSE}
...
```
```{r, echo=TRUE, fig.cap='Punto de corte óptimo con datos de prueba a partir de datos no tratados', warning=FALSE}
...
Se recodifican perdidos en pertenencia indígena.
...
Se recodifican perdidos en nacionalidad.
...
Se recodifican perdidos de razón de inactividad.
...
Las variables de ubicación no tienen valores perdidos.
...
La cantidad de residentes habituales tampoco tienen valores perdidos.
...
Pertenencia indígena y nacionalidad solo se aplica a los miembros del Hogar del Informante Kish. No hay valores perdidos.
...
Las preguntas de ocupación solo se aplican a personas con 15 años o más. Se mantiene esta variable.
...
Primero, se generan vectores con las variables claves y las variables numéricas en el conjunto de datos (nivel hogar).
...
Luego, opcionalmente se pueden declarar variables para el método PRAM (en este caso se genera un vector vacío ya que no se utilizará el método PRAM).
...
```
```{r}
kAnon_violations(sdcHH, weighted = T, k = 2)
kAnon_violations(sdcHH, weighted = T, k = 3)
kAnon_violations(sdcHH, weighted = T, k = 5)
```
...
```{r}
# 2-anonimato
kAnon_violations(sdcCombined, weighted = T, k = 2) 

# 3-anonimato
kAnon_violations(sdcCombined, weighted = T, k = 3) 

# 5-anonimato
kAnon_violations(sdcCombined, weighted = T, k = 5) 
```
...
Primero, revisamos las medidas globales de riesgo con los datos tratados. Se observa una mejora en los resultados, llegando a umbrales aceptables (<10%). No obstante, es necesario revisar el resto de las métricas, donde se está más lejos de cumplir con los estándares requeridos.
...
Luego medimos el *l-diversity*, donde observamos aún valores muy cercanos a 1, para las tres variables sensibles.
...
En conclusión, si bien se cumplen los umbrales a nivel global, los umbrales a nivel individual, el k-anonimato y el *l-diversity* nos indican que se requiere aplicar más métodos SDC para asegurar la anonimización de los datos.
...
Luego medimos el *l-diversity*. Se observa una leve mejora en el *l-diversity*, al observarse ahora una diversidad de 2 en promedio para las distintas combinaciones de variables clave en relación con las variables sensibles.
...
En suma, debido a los riesgos individuales, k-anonimato y *l-diversity*, aún se requiere seguir aplicando métodos SDC.
...
Primero, se recodifican globalmente las variables de pertenencia indígena y nacionalidad. Para esto se ocupa una función distinta, *groupAndRename*, que permite también recodificar variables categóricas.
...
sdcCombined <- groupAndRename(sdcCombined, var="rph_pertenencia_indigena",
                              before=c(1:9), after=c(1))

sdcCombined <- groupAndRename(sdcCombined, var="rph_pertenencia_indigena",
                              before=c(10), after=c(2))
...
También se fusionan categorías de situación laboral, lo que equivale a posteriormente eliminar la variable de razón de inactividad de la base de datos. Esto porque esta variable corresponde a una consolidación de variables (revisar paso 2.1.4).
...
```
```{r}
# Recodificamos condición de inactividad (supone luego eliminar esta variable de los datos finales)
...
sdcCombined <- localSupp(sdcCombined, keyVar='rph_idgen', threshold=0.1)
sdcCombined <- localSupp(sdcCombined, keyVar='rph_pertenencia_indigena', threshold=0.1)
sdcCombined <- localSupp(sdcCombined, keyVar='rph_nacionalidad', threshold=0.1)
sdcCombined <- localSupp(sdcCombined, keyVar='rph_situacion_ocupacional', threshold=0.1)
```
...
Luego medimos el *l-diversity*. Se observa que los valores de promedio y mediana de *l-diversity* son levemente superiores, lo que indican un menor nivel de riesgo de que el intruso logre acertar a los valores de las variables sensibles. Si bien estos no son valores ideales, se decide no proseguir con métodos SDC para no perder más utilidad de los datos. 
...
```
```{r, echo=TRUE, fig.cap='Punto de corte óptimo con datos tratados', warning=FALSE}
plot(ROC.training, colorize=TRUE, print.cutoffs.at=seq(0.1, by=0.1))
abline(a=0,b=1)
abline(v=ptocorteop.training$threshold,col="red")
```
...
```
```{r, echo=TRUE, fig.cap='Punto de corte óptimo con datos de prueba a partir de datos tratados', warning=FALSE}
plot(ROC.testing, colorize=TRUE, print.cutoffs.at=seq(0.1, by=0.1))
abline(a=0,b=1)
abline(v=ptocorteop.testing$threshold,col="red")
```
